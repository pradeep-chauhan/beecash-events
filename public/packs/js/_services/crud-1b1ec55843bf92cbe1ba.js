...
function login(username, password) {
  return function (dispatch) {
    ...
    _services__WEBPACK_IMPORTED_MODULE_1__["userService"].login(username, password).then(function (user) {
      ...
      if (user.status === 200) {
        dispatch(success(user)); // history.push('/app/dashboard');
      }
    }, function (error) {
      dispatch(failure(error.message));
      dispatch(_alert_actions__WEBPACK_IMPORTED_MODULE_2__["alertActions"].error(error.message));
...
function login(email, password) {
  ...
  return _api__WEBPACK_IMPORTED_MODULE_0__["apiConfig"].post("/admin/login", {
    user: user
  }).then(function (data) {
    if (data.status === 200) {
      console.log(data.data); // localStorage.setItem('user', JSON.stringify(data.user.data));
    }
...
/******/ });
//# sourceMappingURL=crud-1b1ec55843bf92cbe1ba.js.map